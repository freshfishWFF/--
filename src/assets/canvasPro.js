export default class CircleProgress{
	...
	drawBase(){
		...
		for(var i = 0; i < this.loop; i++){
    		...
    		this.ctx.lineTo(this.cx+Math.sin(2*Math.PI/this.loop*i)*this.r, this.cy+Math.cos(2*Math.PI/this.loop*i)*this.r);
    		this.ctx.stroke();
    		this.ctx.closePath()
		}
		this.ctx.beginPath();
		this.ctx.arc(this.cx,this.cy,0.7*this.r,0,2*Math.PI);
		this.ctx.fillStyle="#fff";
		this.ctx.fill();
		this.ctx.strokeStyle="#fff"
		this.ctx.beginPath();
		this.ctx.arc(this.cx,this.cy,this.r,0,2*Math.PI);
		this.ctx.stroke();
	}
	drawPer(num){
		num = num || 0;
	  ...
	  this.ctx.textAlign = 'center';//设置文本的水平对齐方式
		this.ctx.strokeText(num,this.cx,this.cy);
		this.ctx.strokeStyle="#fff"
		this.ctx.beginPath();
		this.ctx.arc(this.cx,this.cy,this.r,0,2*Math.PI);
		this.ctx.stroke();
